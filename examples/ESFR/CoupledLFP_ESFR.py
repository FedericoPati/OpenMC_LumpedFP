"""CoupledLFP_ESFR.py — Coupled MG transport + depletion with LFP lumping (ESFR pin).

Step-5 validation against the ESFR CE reference (Analysis/AnalysisStep5/DepletionCE_fast):
same physical pin-cell (annular fuel with inner He hole, Pu-rich ESFR loading,
power 335 W, edge 0.67377 cm) and same burnup schedule as the CE run, but using
the LFP MGXS library and the custom depletion chain that lumps all fission
products into one LFP carrier per fissile parent.

Mirrors the structure of Analysis/AnalysisStep4/CoupledLFP.py but uses the plain
CoupledDepletionDriver (no predictor-corrector) and ESFR-specific conditions.

Results are stored in run/ for subsequent analysis.
"""
...
LIB_ROOT   = THESIS / "library" / "MGXS_Library_old"
OUT_DIR    = Path(__file__).resolve().parent / "run"

POWER_W            = 335.0     # W — matches CE DepletionCE_fast/runESFR.py
FUEL_VOLUME        = 0.61163   # cm³
FUEL_TEMPERATURE   = 1500.0    # K
STRUCT_TEMPERATURE = 673.0     # K

# Cumulative burnup targets [MWd/kgHM] — built from the CE schedule
# dt = (1, 2, 4, 6, 7, 10, 10, 10, 10, 10, 10, 10, 10) MWd/kg (incremental)
BURNUP_TARGETS = [
    1.0,  3.0,  7.0, 13.0, 20.0,
   30.0, 40.0, 50.0, 60.0, 70.0,
   80.0, 90.0, 100.0,
]

# ESFR pin-cell geometry (annular fuel with central He hole)
INNER_HOLE_R = 0.1560   # cm
FUEL_R       = 0.4680   # cm
GAP_R        = 0.4835   # cm
CLAD_R       = 0.5358   # cm
EDGE         = 0.67377  # cm
...
def build_initial_fuel() -> openmc.Material:
    """ESFR Pu-rich fuel (same composition as DepletionCE_fast/runESFR.py)."""
    m = openmc.Material(material_id=1, name="fuel")
    m.add_nuclide("U235",  4.72863E-05 , "ao")
    m.add_nuclide("U238",  1.86289E-02 , "ao")
    m.add_nuclide("Pu238", 1.46306E-04, "ao")
    m.add_nuclide("Pu239", 1.93325E-03, "ao")
    m.add_nuclide("Pu240", 1.20500E-03 , "ao")
    m.add_nuclide("Pu241", 3.33022E-04, "ao")
    m.add_nuclide("Pu242", 4.18070E-04, "ao")
    m.add_nuclide("Am241", 3.15180E-05 , "ao")
    m.add_nuclide("O16",   4.50318E-02 , "ao")
    m.depletable  = True
...
def geometry_builder(materials: list[openmc.Material]) -> openmc.Geometry:
    """ESFR annular pin: He hole + fuel + He gap + clad + Na coolant."""
    fuel_m, gap_m, clad_m, cool_m = materials

    inner_hole_s = openmc.ZCylinder(r=INNER_HOLE_R)
    ...
    pin_u = openmc.model.pin(
        surfaces=[inner_hole_s, fuel_s, gap_s, clad_s],
        items=[gap_m, fuel_m, gap_m, clad_m, cool_m],
    )
...
def settings_builder() -> openmc.Settings:
    """MC settings matching DepletionCE_fast/runESFR.py: 350 batches × 100000 particles."""
    s = openmc.Settings()
    ...
    entropy_mesh.lower_left  = (-EDGE, -EDGE, -0.5)
    entropy_mesh.upper_right = ( EDGE,  EDGE,  0.5)
    s.entropy_mesh = entropy_mesh
